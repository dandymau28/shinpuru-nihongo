...
import type { useConjStats } from "@/lib/conjPractice";
import { Card } from "@/components/ui/Card";

export function ConjugationStatsPanel({
  stats,
}: {
  stats: ReturnType<typeof useConjStats>;
}) {
  const { t } = useSettings();
        ...
        {ranked.slice(0, 6).map((r) => (
          <li key={r.id} className="flex items-center gap-2 text-xs">
            <span className="w-40 shrink-0 truncate">
              {r.form && <span aria-hidden>{r.form.emoji} </span>}
              {r.form && t(r.form.label)}
            </span>
