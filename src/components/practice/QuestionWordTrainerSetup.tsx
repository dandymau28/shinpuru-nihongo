...
export function QuestionWordTrainerSetup({
  settings,
  onChange,
  onStart,
}: {
  settings: QWSettings;
  onChange: (s: QWSettings) => void;
  onStart: () => void;
}) {
  const { t } = useSettings();
  const [pickOpen, setPickOpen] = useState(false);
  const set = (patch: Partial<QWSettings>) => onChange({ ...settings, ...patch });

  const activePreset = PRESETS.find(
    (p) =>
      p.contrast === settings.contrast &&
      sameSet(p.words, settings.words) &&
  ...
  const toggleWord = (w: string) => {
    set({
      contrast: null,
      words: settings.words.includes(w)
        ? settings.words.filter((x) => x !== w)
        : [...settings.words, w],
    });
  };
    ...
    <div className="space-y-4">
      <Card className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-sm font-semibold">{t(STR.qw_pick_what)}</h2>
          <button
          ...
          <div className="space-y-2 rounded-xl border border-border p-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {QUESTION_WORDS.map((w) => {
                const on = settings.words.includes(w);
                return (
                  <button
                    key={w}
